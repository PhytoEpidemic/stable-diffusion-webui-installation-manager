...
local function setSetting(param, value)
	...
	if param:sub(1,#"COMMANDLINE_ARGS") == "COMMANDLINE_ARGS" then
		...
		if installLocation then
			update_value_in_file(installLocation.."\\webui\\webui-user.bat",param,value)
		end
	else
...
local function getSetting(param)
	if param:sub(1,#"COMMANDLINE_ARGS") == "COMMANDLINE_ARGS" then
		local installLocation = get_value_from_file(settingsFileLocation,"installLocation")
		if installLocation then
			return get_value_from_file(installLocation.."\\webui\\webui-user.bat",param)
		end
	else
...
--result = get_nested_value(split_string(arg[1]), nested_table)
